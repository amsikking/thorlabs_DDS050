...
class Controller:
    '''
    Basic device adaptor for thorlabs DDS050 compact 50 mm direct drive stage.
    - Uses a KBD101 K-Cube brushless DC servo driver (controller)
    Test code runs and seems robust.
    '''
    def __init__(
        self, which_port, name='DDS050', verbose=True, very_verbose=False):
        self.name = name
...
if __name__ == '__main__':
    stage = Controller('COM11', verbose=True, very_verbose=False)
...
##    stage.identify()
    ...
    print('\n# Test max range:')
    stage.move_mm(50, relative=False)
    stage.move_mm(0, relative=False)

    print('\n# Some relative moves:')
    for moves in range(3):
        move = stage.move_mm(10)
    for moves in range(3):
        move = stage.move_mm(-10)
    ...
    for moves in range(3):
        random_move_mm = uniform(0, 50)
        move = stage.move_mm(random_move_mm, relative=False)

    print('\n# Non-blocking move:')
    stage.move_mm(10, relative=False, block=False)
